...
if (empty($_SESSION["username"]) || empty($_SESSION["password"])) {
    ...
    while ($r1 = mysqli_fetch_assoc($wisata)) {
    $nama_wisata[] = $r1['nama_wisata'];
    $jml_wisata = mysqli_query($koneksi, "SELECT COUNT(id_wisata) AS total FROM tbl_galeri WHERE id_wisata = {$r1['id_wisata']}");
    $a1 = mysqli_fetch_assoc($jml_wisata);
    $total_galeri[] = $a1['total'];
}
?>
    ...
    $(document).ready(function () {
        ...
        t.on('order.dt search.dt', function () {
            ...
            options: {
                title: {
                display: true,
                text: "Jumblah Data Wisata Berdasarkan Kategori "
                    }
                }
                });

            var x1 = <?php echo json_encode($nama_wisata)?>;
            var y1 = <?php echo json_encode($total_wisata)?>;
            var warna_bar1 = [
            "#b91d47",
            "#00aba9",
            "#2b5797",
